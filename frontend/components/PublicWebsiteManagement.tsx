  ...
  const loadPublicProfile = async () => {
    try {
      ...
      if (response.success) {
        ...
        const mappedProfile = {
          id: data.id || data.agent_id || 'current-agent',
          agent_name: data.agent_name || `${data.first_name || ''} ${data.last_name || ''}`.trim() || 'Agent',
          slug: data.slug || data.agent_name?.toLowerCase().replace(/\s+/g, '-') || 'agent',
          bio: data.bio || data.professional_bio || '',
          phone: data.phone || '',
          email: data.email || '',
          office_address: data.office_address || '',
          specialties: data.specialties || (data.specialization_areas ? data.specialization_areas.split(',').map(s => s.trim()) : []),
  ...
  const loadStats = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Set default stats on API failure
        setStats({
          total_views: 0,
          total_contacts: 0,
          properties_count: 0,
