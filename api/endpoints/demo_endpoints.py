...
@router.get("/api/demo/leads")
async def get_demo_leads(authorization: Optional[str] = Header(None)):
    """Get demo leads data - working endpoint"""
    
    # Get current user (unified auth)
    user = await get_current_user_unified(authorization)
...
async def create_demo_lead(request: Request, authorization: Optional[str] = Header(None)):
...
@router.get("/api/demo/properties")
async def get_demo_properties(authorization: Optional[str] = Header(None)):
...
@router.post("/api/demo/properties") 
async def create_demo_property(request: Request, authorization: Optional[str] = Header(None)):
...
async def get_demo_dashboard(authorization: Optional[str] = Header(None)):
