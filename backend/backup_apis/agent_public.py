...
@router.post("/{agent_slug}/update-branding")
async def update_agent_branding(
    ...
    """
    Update agent's branding data (temporary fix for missing branding from onboarding)
    """
    try:
        service = AgentPublicService(db)
        ...
        # Update branding data in the database
        ...
        if update_result.modified_count == 0:
            raise HTTPException(status_code=400, detail="Failed to update branding data")
        
        return {"message": "Branding data updated successfully", "agent_slug": agent_slug}
